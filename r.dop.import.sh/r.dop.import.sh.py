...
############################################################################
#
# MODULE:      r.dop.import.sh
# AUTHOR(S):   Johannes Halbauer, Anika Weinmann, Leon Louwarts
# PURPOSE:     Downloads DOPs for Schleswig-Holstein and AOI
...
# %module
# % description: Downloads DOPs for Schleswig-Holstein and AOI.
# % keyword: raster
# % keyword: import
# % keyword: DOP
...
TINDEX = (
    "https://github.com/mundialis/tile-indices/raw/main/DOP/SH/"
    "DOP20_tileindex_SH.gpkg.gz"
)
...
def main():
    ...
    try:
        grass.message(
            _(f"Importing {number_tiles} DOPs for SH in parallel..."),
        )
        for tile in url_tiles:
            ...
            r_dop_import_worker_sh = Module(
                "r.dop.import.worker.sh",
                **param,
                run_=False,
            )
            # catch all GRASS output to stdout and stderr
            r_dop_import_worker_sh.stdout_ = grass.PIPE
            r_dop_import_worker_sh.stderr_ = grass.PIPE
            queue.put(r_dop_import_worker_sh)
        queue.wait()
    except Exception:
