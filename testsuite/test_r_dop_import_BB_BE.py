...
class TestRDopImport(TestCase):
    pid = os.getpid()
    rm_vec = []
    aoi_map_file = os.path.join("data", "test_aoi_BB.gpkg")
    ...
    def test_extent_aoi_map(self):
        ...
        self.assertTrue(
            int(cells_aoi) == 128928, "Data loaded for larger region than AOI"
        )
        print("Test AOI (BB/BE) successfully finished.\n")

    def test_dop_resolution(self):
        """
        Tests importing data with original resolution of DOPs
        """
        print("\nTest resolution (BB/BE) ...")
        r_check = SimpleModule(
            "r.dop.import",
            output=self.test_output,
            aoi_map=self.aoi_map,
        ...
        print("Test resolution (BB/BE) successfully finished.\n")
