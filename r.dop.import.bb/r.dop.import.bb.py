...
############################################################################
#
# MODULE:      r.dop.import.bb
# AUTHOR(S):   Johannes Halbauer, Anika Weinmann
#
# PURPOSE:     Downloads DOPs for Brandenburg and AOI
...
# %module
# % description: Downloads DOPs for Brandenburg and AOI.
# % keyword: raster
# % keyword: import
# % keyword: DOP
...
def main():
    ...
    global temp_loc_path, keep_data, download_dir
    ...
    tmp_loc, srcgisrc = create_tmp_location(25833)

    # reproject aoi to temp location
    grass.run_command(
        "v.proj",
        location=loc,
        mapset=mapset,
    ...
    for url in url_tiles:
        ...
        # import DOPs with overriding projection check (o flag)
        grass.run_command(
            "r.import",
            input=input,
            output=dop_name,
    ...
    proj_res = grass.parse_command(
        "r.info",
        map=all_dops[0][0],
        flags="g",
    )["nsres"]

    # save rm path of temp location
    temp_loc_path = (
        grass.gisenv()["GISDBASE"] + "/" + grass.gisenv()["LOCATION_NAME"]
    )
    ...
    # reproject imported rasters
    for band in all_dops:
        for raster in band:
            grass.run_command(
                "r.proj",
                location=tmp_loc,
